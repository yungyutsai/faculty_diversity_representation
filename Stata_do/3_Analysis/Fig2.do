...
twoway 	(line is_D7 year, lc(navy)) ///
		(line is_D9 year if year > 2009, lc(maroon) lp(dash)), ///
		xlabel(2001(2)2021) ///
		ylabel(0.3(0.05)0.55,angle(0) format(%4.2f)) scheme(s1color) ///
		ytitle("Faculty Diversity Index") xtitle(Year) ///
		legend(order(1 "Based on 7-bin Scale" 2 "Based on 9-bin Scale") size(small))
graph export "$figure/Fig2a_Diversity.png", replace


twoway 	(line is_R7 year, lc(navy)) ///
		(line is_R9 year if year > 2009, lc(maroon) lp(dash)), ///
		xlabel(2001(2)2021) ///
		ylabel(,angle(0) format(%4.2f)) scheme(s1color) ///
		ytitle("Representation Index") xtitle(Year) ///
		legend(order(1 "Based on 7-bin Scale" 2 "Based on 9-bin Scale") size(small))
graph export "$figure/Fig2b_Representation.png", replace
...
foreach var in  is_D7 is_R7{ //fte_D7
	forv i = 1(1)6{
	...
	graph export "$figure/Fig2cd_`name'_byTypes.png", replace
}
